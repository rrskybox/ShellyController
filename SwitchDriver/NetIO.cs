...
namespace ASCOM.ShellyRelayController.Switch
{
    public class NetIO
    {
        ...
        internal Uri ShellyIPAddress;
        internal HttpClient ShellyClient;
        internal int ShellyPort = 80;

        public NetIO(string ipAddress)
        {
            ShellyIPAddress = new Uri("http://" + ipAddress+"/rpc/");
            ShellyClient = new HttpClient()
            {
                BaseAddress = ShellyIPAddress
            };
        ...
        public string SendCommand(string command)
        {
            ...
            try
            {
                response = ShellyClient.PostAsync(ShellyClient.BaseAddress,content).Result;
            }
            catch (Exception ex)
            {
                System.Windows.Forms.MessageBox.Show("NT - SendCommand " + ex.Message);
            }
